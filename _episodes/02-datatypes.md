...
know what your own programs and scripts do when you open them
again after spending several months finishing
some other project.
...
Anaconda helps install many important Python packages. You can use `conda` to install others that do not come with the default install. One very useful Python library for bioinformatics is [biopython](http://biopython.org/). You can install this easily with `conda` from your terminal window.
...
One package [`string-color`](https://pypi.org/project/string-color/) allows you to print strings to screen in different colors.
